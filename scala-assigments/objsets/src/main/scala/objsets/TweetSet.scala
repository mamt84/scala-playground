...
class NonEmpty(elem: Tweet, left: TweetSet, right: TweetSet) extends TweetSet {
  ...
  def filterAcc(p: Tweet => Boolean, acc: TweetSet): TweetSet = {
    if (!p(elem))
      ...
      left.filterAcc(p, acc union new NonEmpty(elem, new Empty, new Empty)) union right.filterAcc(p, acc union new NonEmpty(elem, new Empty, new Empty))
  }

  def union(that: TweetSet): TweetSet = left union ((right union that) incl elem)

  def mostRetweeted: Tweet = {
    val remainingMostRetweeted = this.remove(elem).mostRetweeted
    if (remainingMostRetweeted == null || elem.retweets > remainingMostRetweeted.retweets)
      elem
    else
      remainingMostRetweeted
  }
